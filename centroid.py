...
def form_cluster(coords, metric, epsilon):
	...
	#db = DBSCAN(eps=epsilon, min_samples=1, algorithm='ball_tree', metric=metric).fit(np.radians(coords))
	db = DBSCAN(eps=epsilon, min_samples=1, algorithm='ball_tree', metric=metric).fit(coords)
